...
- Mises à jour internes (start-sdk 2.0).`,
  ...
  migrations: {
    up: async ({ effects }) => {
      const externalip = (
        await floweeConfFile.read((c) => c.raw?.externalip).once()
      )?.filter((ip) => !!ip && !ip.includes('.onion'))

      await floweeConfFile.merge(effects, {
        raw: {
          // Earlier revisions of this package wrote a key the Hub renamed to
          // apibind, and onion addresses it refuses to parse at startup. The
          // plaintext rpcuser/rpcpassword pair they also wrote is cleared by
          // the file model itself on the next write.
          apilisten: undefined,
          externalip: externalip?.length ? externalip : undefined,
        },
      })

      // The credentials these held are superseded by hashed rpcauth entries,
      // and two of them were plaintext passwords.
      await storeJson.merge(effects, {
        rpcCredentials: undefined,
        rpcUser: undefined,
        rpcPassword: undefined,
        initialized: undefined,
        torEnabled: undefined,
      })
    },
    down: IMPOSSIBLE,
  },
