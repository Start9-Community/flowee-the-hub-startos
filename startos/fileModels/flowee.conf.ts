...
export const shape = z
  .object({
    ...
    // Legacy keys the package used to write; cleared by the current migration
    // and kept here only so an unmigrated file still parses.
    proxy: iniString,
    onion: iniString,
...
function formToFile(
  ...
  return {
    ...
    // their entries as possibly absent.
    rpcauth: raw?.rpcauth?.filter((a): a is string => !!a),
    externalip: raw?.externalip?.filter((a): a is string => !!a),
    server: true,
    listen: true,
    rpcbind: '0.0.0.0',
    ...
    'use-thinblocks': true,
    'min-thin-peers': 2,
    // proxy / proxyrandomize are daemon arguments, driven by store.json.
    proxy: undefined,
    proxyrandomize: undefined,
    onion: undefined,
    listenonion: undefined,
  }
}
